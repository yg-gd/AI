...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #                                    if livingGhosts[i+1]]
        livingGhostPositionDistributions = [beliefs for beliefs in self.ghostBeliefs]
        "*** YOUR CODE HERE ***"

        mlPositions = {}
        for ghost in range(1,len(livingGhosts)):
            if not livingGhosts[ghost]:
                continue
            bestProb = 0
            #if len(livingGhostPositionDistributions)is 1:
            #    print ghost
            dist = livingGhostPositionDistributions[ghost-1]
            for loc,prob in dist.iteritems():
                if prob > bestProb:
                    bestProb = prob
                    mlPositions[ghost] = loc

        bestAction = None
        closestDistance = None
        for action in legal:
            succPos = Actions.getSuccessor(pacmanPosition, action)
            for ghost,loc in mlPositions.iteritems():
                dist = self.distancer.getDistance(succPos, loc)
                if closestDistance > dist or closestDistance is None:
                    bestAction = action
