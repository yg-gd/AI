    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for newPos in self.legalPositions:
            ...
            count = 0
            "old prob isn't an x,y pair"
            for oldPos, oldProb in self.beliefs.iteritems():
                if oldProb >0 :
                    newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
                    count = count + newPosDist[newPos] * self.beliefs[oldPos]
            newbelief[newPos] = count  
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [self.legalPositions[p%len(self.legalPositions)] for p in range(0,self.numParticles)]
    def observe(self, observation, gameState):
        """
        ...
        for p in self.particles:
            weight[p] += emissionModel[util.manhattanDistance(p,pacmanPosition)]
        if weight.totalCount() == 0:
            self.initializeUniformly( gameState)
        else:
           # newparticle = []
           # for p in self.particles:
           #     newparticle.append(util.sample(weight))
            newparticle = util.nSample(weight.values(),weight.keys(),self.numParticles)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in range(self.numGhosts):
            legPos[i] = self.legalPositions[:]
            random.shuffle(legPos[i])

        possible = list(itertools.product(*legPos.values()))

        self.particles = [possible[i%len(possible)] for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                if newParticle[i] == self.getJailPosition(i):
                    continue
               # if newParticle[i] in gameState.data.layout.walls.asList():
               #     print "before elapse" 
                state = setGhostPositions(gameState, list(oldParticle))
    ...
    def getBeliefDistribution(self):
        ...
        for i in range(self.numGhosts):
            ...
            for particle in self.particles:
                count[particle[i]] += 1
            ghostDist[i] = util.normalize(count)
        return ghostDist
        '''
